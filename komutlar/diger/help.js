...
module.exports = {
  ...
  run: async (client, message, args) => {
    const embed = new EmbedBuilder()
      .setColor("#000001")
      .setAuthor({
        name: `${message.guild.members.me.displayName} Yardım komutu!`,
    ...
    if (!args[0]) {
      ...
      embed.setFooter({
        text: `© ${message.guild.members.me.displayName} | Toplam Komut: ${client.commands.size}`,
      ...
      categories.forEach((kategori) => {
        ...
        const capitalise =
          kategori.slice(0, 1).toUpperCase() + kategori.slice(1);
        try {
          embed.addFields({
        ...
        return message.channel.send({
          embeds: [
            embed
              ...
              .setDescription(
                `Komut listesini görmek için \`${client.prefix}help\` yazınız!`
              ),
          ],
        });
